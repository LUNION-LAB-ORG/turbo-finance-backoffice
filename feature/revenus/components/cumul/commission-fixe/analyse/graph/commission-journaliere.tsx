...
interface CommissionJournaliereProps {
    commissionFixe?: ICommission[]; // Rendre la prop optionnelle
}

// Fonction pour grouper les dépenses par jour de la semaine
const groupCommissionByDay = (commissionFixe: ICommission[] = []) => {
    ...
    if (!commissionFixe || !Array.isArray(commissionFixe)) {
        return jours.map(j => ({ day: j.nom, commission: j.commission }))
    }

    // Parcourir toutes les commissions et les grouper par jour de la semaine
    commissionFixe.forEach(commissionItem => {
        try {
...
const chartConfig = {
    commissionFixe: {
        label: "Commission Fixe journalières",
        color: "hsl(47, 81.50%, 48.80%)", 
    },
} satisfies ChartConfig

export function CommissionJournaliereChart({ commissionFixe = [] }: CommissionJournaliereProps) {
    // Transformer les données des dépenses en format pour le graphique
    const chartData = useMemo(() => {
        return groupCommissionByDay(commissionFixe)
    }, [commissionFixe])

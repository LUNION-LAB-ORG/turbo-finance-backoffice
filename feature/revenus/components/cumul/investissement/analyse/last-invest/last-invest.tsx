...
interface LastInvestissementProps {
    investissements?: IInvestissement[];
}

export default function LastInvestissement({ investissements }: LastInvestissementProps) {
    
    // Filtrer les investissements du mois courant
    const investissementsMoisCourant = useMemo(() => {
        if (!investissements) return [];
        ...
        return investissements.filter(investissement => {
            const dateInvest = new Date(investissement.dateInvestissement);
                    ...
                    <div className="p-5">
                        ...
                        >
                            <CarouselContent className="h-[400px]">
                                {investissementsMoisCourant?.map((investissement) => (
                    ...
                    <div className="px-5 py-4 border-t border-gray-200 bg-gray-50">
                        <div className="flex justify-between items-center">
                            <div className="text-sm text-gray-600">
                                Total ce mois: {investissementsMoisCourant?.length} investissement(s)
                            </div>
                            <div className="text-sm font-semibold text-blue-600">
                                Montant total: {investissementsMoisCourant?.reduce((sum, invest) => sum + invest.montant, 0).toLocaleString()} FCFA
                            </div>
                        </div>
                    </div>
                </CardContent>
